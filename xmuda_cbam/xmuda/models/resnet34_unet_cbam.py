...
class UNetResNet34(nn.Module):
    def __init__(self, pretrained=True, num_classes=None, feature_dim=64):
    ...
    def forward(self, x):
        ...
        # Project to 64D and apply CBAM
        feat = self.feature_proj(x)  # [B,512,H,W]
        feat = self.cbam(feat)  # 应用注意力

        if self.classifier is not None:
            seg_output = self.classifier(feat)  # 用于监督训练
            return seg_output, feat
        else:
            return feat  # 用于提特征融合
